...
static int snp_launch_update_upm(struct kvm *kvm, struct kvm_sev_cmd *argp)
{
	...
	for (i = 0; i < npages; i++) {
		...
		/* TODO: put the previous PFNs if a failure occurs. */
		if (kvm_private_mem_get_pfn(slot, gfn, &pfns[i], &order)) {
...
e_release:
	...
	for (i = 0; i < npages; i++) {
		struct kvm_memory_slot *slot = gfn_to_memslot(kvm, params.start_gfn + i);

		kvm_private_mem_put_pfn(slot, pfns[i]);
	}
...
/* TODO: leave this as no-ops for now. This will result in subsequent
 * RMP faults that hopefully handle_rmp_page_fault() can handle the
 * rmpupdates for. Ideally these will get fixed up in bulk here though
 * eventually to avoid unecessary guest exits.
 */
